...
void TemplateFitter(const char* dataFile="data.root", const char* dataHist="hist",
  ...
  RooRealVar x("x","BDT_bscore", xmin, xmax);
  ...
  RooRealVar fC("fC","fraction C", 0.03, 0.0, 1.0);
  ...
  RooFitResult *res = model.fitTo(dataHistR, Extended(kTRUE), Save(), SumW2Error(kTRUE));
  ...
  frame->GetXaxis()->SetTitle("BDT b score");
...
pad2->cd();
  ...
  hRatio->GetXaxis()->SetTitle("BDT b score");
  ...
  c->SaveAs("BDT_b_score_template_fit.pdf");
  ...
  TFile *fitresult = new TFile("fitresult1D.root", "recreate");
  fitresult->cd();
